...
def main():
    ...
    for step in range(num_steps):
        ...
        if step % 50 == 0:
            ...
            frame_path = save_frame(step, positions, thetas, box, fig, ax, "frames"+"_v_"+str(v)+"_Dr_"+str(Dr)+"_Dt_"+str(Dt)+"_seed_"+str(seed)+"_nump_"+str(num_particles)+"_box_"+str(box), areas, dpi)
            frames.append(frame_path)

    plt.close(fig)
    output_path = "output"+"_v_"+str(v)+"_Dr_"+str(Dr)+"_Dt_"+str(Dt)+"_seed_"+str(seed)+"_nump_"+str(num_particles)+"_box_"+str(box)+".mp4"
